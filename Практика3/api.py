...
def get_model_results():
    ...
    for name, model in models.items():
        ...
        # --- AutoML (FLAML) ---
        automl_model = AutoML()
        automl_model.fit(X_train, y_train, task='classification', time_budget=60)
        automl_y_pred = automl_model.predict(X_val)
        automl_acc = accuracy_score(y_val, automl_y_pred)
        automl_prec = precision_score(y_val, automl_y_pred)
        automl_rec = recall_score(y_val, automl_y_pred)
        automl_f1 = f1_score(y_val, automl_y_pred)
        automl_auc = roc_auc_score(y_val, automl_model.predict_proba(X_val)[:, 1]) if hasattr(automl_model,
                                                                                              'predict_proba') else "N/A"

        # Add AutoML results
        results["AutoML"] = [automl_acc, automl_prec, automl_rec, automl_f1, automl_auc]
...
def model_visualization():
    ...
    # --- AutoML (FLAML) ---
    automl_model = AutoML()
    automl_model.fit(X_scaled, y, task='classification', time_budget=60)  # Задайте время на обучение
    models["AutoML"] = automl_model

    # Сгенерировать изображение для модели AutoML
    img_base64_automl = generate_plot_image(automl_model, X_scaled, y, selected_features)
    images["AutoML"] = img_base64_automl
